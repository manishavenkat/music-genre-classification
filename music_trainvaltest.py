...
def train_model(model, train_loader, val_loader, criterion, optimizer, max_epochs=1000, patience=10):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('loss_plot_music_trainvaltest.png')
    plt.close()
...
def evaluate_model(model, dataloader, classes):
    ...
    plt.ylabel('True')
    plt.savefig('confusion_matrix_music_trainvaltest.png')
    plt.close()
...
def create_tsne_plot(model, dataloader, classes):
    ...
    plt.savefig('tsne_plot_music_trainvaltest.png')
    plt.close()

print('Making T-SNE')
create_tsne_plot(model, test_loader, classes)
